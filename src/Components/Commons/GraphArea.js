  ...
  return (
    <GraphWrapper>
      <HeaderSpreader>        
        ...
        <GraphStockValues>
        <img className="stockStatus" src={Boolean(!change) ? '' : (upOrDown === -1 || upOrDown === -0? graphdown : graphup) } alt=''/>        
        <FontStyle symbol>{latestPrice && companyName? `$${latestPrice}` : ''}</FontStyle>
        {upOrDown === -1 || upOrDown === -0? 
            (<FontValues>{change && companyName? `$${change} (${changePercent}%)` : ''}</FontValues>)   
