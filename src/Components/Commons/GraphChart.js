...
const GraphChart = ({ companySymbol }) => {
  ...
  useEffect(() => {
    const getChart = async () => {
      try {
        const response = await iex.get(`/${companySymbol}/chart/1m`);
        setGraphChartData(response.data);
      } catch (err) {
        console.log(err);
      }
    };
    return companySymbol ? getChart() : null;
  }, [companySymbol]);
  ...
  return (
    <>
      ...
      >
        <input
          type="checkbox"
          defaultChecked={graphPrice}
          onChange={() => setGraphPrice((s) => !s)}
          name="showprice"
        />
